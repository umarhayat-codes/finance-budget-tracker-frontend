  ...
  return (
    <div className="bg-transactionBg border-[0.85px] border-transactionBorder rounded-[10px] p-6 w-full">
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-grow md:flex-grow-0">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 bg-transactioncolor border border-transactionBorder rounded-[8px] text-[12px] font-satoshi w-full md:w-[220px] outline-none"
            />
          </div>
        ...
        <table className="w-full text-left border-separate border-spacing-0">
          ...
          <tbody className="bg-white">
            {transactions.map((item) => (
              <tr
                key={item.id}
                className="group hover:bg-bgColor transition-colors"
              >
                <td className="px-4 py-4 border-b border-l border-transactionBorder">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-transactionTableHeaderBg flex items-center justify-center">
                ...
                <td className="px-4 py-4 border-b border-r border-transactionBorder text-right relative">
                  <button
                    ...
                    className="p-1 hover:bg-exportBorder rounded-full transition-colors text-transactionTextBrand"
                  >
                    <FiMoreVertical />
                  </button>
                  {activeMenuId === item.id && (
                    <div className="absolute right-4 top-12 z-10 bg-clarioWhite border border-transactionBorder rounded-lg shadow-lg py-1 w-24 text-left">
                      <button className="w-full px-4 py-2 text-[12px] font-satoshi text-transactionTextBrand hover:bg-exportBg">
                        Update
                      </button>
                      <button className="w-full px-4 py-2 text-[12px] font-satoshi text-transactionNegative hover:bg-exportBg">
                        Delete
                      </button>
                    </div>
                  )}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
        ...
        <div className="flex items-center gap-2">
          <button className="p-2 border border-transactionBorder rounded-full text-transactionTextMuted hover:bg-exportBg transition-colors">
            <FiChevronLeft />
          </button>
            ...
            {[1, 2, 3, "...", 9].map((page, i) => (
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-full font-satoshi text-[12px] transition-colors ${
                  page === 1
                    ? "bg-transactionActiveBg text-white"
                    : "text-transactionTextMuted hover:bg-exportBg"
                }`}
              >
                {page}
              </button>
